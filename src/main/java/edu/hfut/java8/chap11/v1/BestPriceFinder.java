package edu.hfut.java8.chap11.v1;
...
import edu.hfut.java8.chap11.ExchangeService;
import edu.hfut.java8.chap11.ExchangeService.Money;
...
public class BestPriceFinder {
	...
	public List<String> findPricesInUSD2(String product) {
		...
		for (Shop shop : shops) {
			...
			CompletableFuture<String> futurePriceInUSD = CompletableFuture
					...
					.thenCombine(CompletableFuture.supplyAsync(() -> ExchangeService.getRate(Money.EUR, Money.USD)),
							(price, rate) -> price * rate).thenApply(price -> shop.getName() + " price is " + price);
			priceFutures.add(futurePriceInUSD);
		}
		List<String> prices = priceFutures.stream().map(CompletableFuture::join).collect(Collectors.toList());
	...
	public List<String> findPricesInUSD3(String product) {
		...
		Stream<CompletableFuture<String>> priceFuturesStream = shops.stream().map(
